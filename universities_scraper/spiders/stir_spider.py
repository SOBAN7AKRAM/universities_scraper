...
class StirSpider(scrapy.Spider):
    name = "stir_spider"
    
    def __init__(self, name=None, **kwargs):
        super().__init__(name, **kwargs)  # Ensure proper superclass initialization
        self.base_url = "https://www.stir.ac.uk/staff-profiles/"
        self.output_folder = "urls"
        self.html_filename = "stir.txt"
    ...
    async def parse(self, response):
        ...
        try:
            while True:
                ...
                try:
                    button = await page.wait_for_selector(
                        "button.button.hollow.tiny:has-text('Load more results')",
                        state="visible",
                        timeout=5000
                    )
            ...
            profile_links_elements = soup.select("#search_results_panel_staff a")
