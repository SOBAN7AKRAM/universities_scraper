...
class Hw2Spider(scrapy.Spider):
    ...
    async def extract_emails(self, response):
        ...
        try:
            # Wait for the email element to be available (ensure scripts are executed)
            await page.wait_for_selector("a.email")

            # Get the text content of the element (which should be the complete email)
            email = await page.inner_text("a.email")
            if email in self.seen_emails:
                return
            self.seen_emails.add(email)
            yield {"email": email, "university": "hw.ac.uk"}
